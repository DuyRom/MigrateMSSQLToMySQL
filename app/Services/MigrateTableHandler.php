    ...
    public static function migrateTables()
    {
        ...
        foreach ($tables as $table) {
            ...
            try {
                ...
                $totalMigrated = 0;
                if (Schema::connection('sqlsrv')->hasColumn($table->TABLE_NAME, 'id')) {
                    DB::connection('sqlsrv')->table($table->TABLE_NAME)->orderBy('id')->chunk(1000, function ($rows) use ($tableName, &$totalMigrated) {
                        foreach ($rows as $row) {
                            DB::connection('mysql')->table($tableName)->insert((array) $row);
                            $totalMigrated++;
                        }
                    ...
                    $firstColumn = Schema::connection('sqlsrv')->getColumnListing($table->TABLE_NAME)[0];

                    DB::connection('sqlsrv')->table($table->TABLE_NAME)->orderBy($firstColumn)->chunk(1000, function ($rows) use ($tableName, &$totalMigrated) {
                        foreach ($rows as $row) {
                            DB::connection('mysql')->table($tableName)->insert((array) $row);
                            $totalMigrated++;
                        }
